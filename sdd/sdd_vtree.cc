...
void VTree::setup_node_depth() {
    ...
    std::cout<< "tree_size: "<<tree_nodes_.size()<<std::endl;
    // find the root 
    int root_idx = -1;
    for (int i = 0; i < (int)tree_nodes_.size(); i++) {
        const VTreeNode& n = tree_nodes_[i];
        std::cout<< "i: " << i << "parent: " << n.parent()<< std::endl;
        if (n.parent() < 0) {
            root_idx = i;
            break;
        }
    }
    std::cout << "vtree.cc file: root_idx:" <<root_idx<<"  tree_nodes_size: "<< (int)tree_nodes_.size()<<std::endl; 
...
int VTree::get_depend_node(const int lhs_id, const int rhs_id) const {  //公共祖先节点
    //std::cout << "lhs_id="<<lhs_id<<"  rhs_id="<<rhs_id<<std::endl;
    //std::cout << "tree_nodes_size="<<(int)tree_nodes_.size()<<std::endl;
     assert(lhs_id >= 0 && lhs_id < (int)tree_nodes_.size() && rhs_id >= 0 && rhs_id < (int)tree_nodes_.size());
...
VTree VTree::import_from_sdd_vtree_file(const std::string& file_name) {
    ...
    while (getline(ifs, line)) {
        ...
        if (on_reading_header) {
            ...
            is >> type;
            is >> num_nodes;
            std::cout<<"vtree read num node 1:"<<num_nodes<<std::endl;
            if (num_nodes == 0)
            is >> num_nodes;
            std::cout<<"vtree read num node 2:"<<num_nodes<<std::endl;
            assert (type== "vtree");
...
// VTree VTree::construct_balanced_vtree(const unsigned int num_vars)
// {
//     std::vector<VTreeNode> vtree_nodes;
